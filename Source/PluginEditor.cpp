...
void SimpleEQAudioProcessorEditor::paint (juce::Graphics& g)
{
    ...
    g.fillAll(Colour::fromRGB(20, 22, 26));
    ...
    auto map = [outputMin, outputMax](double input) {
        return jmap(input, -24.0, 24.0, outputMin, outputMax); // input is -24 and 24 because peak control can go to -24 and +24 so the response curve has the same with this
    };
    ...
    g.drawRoundedRectangle(responseArea.toFloat(), 4.f, 1.f); // draws rounded rectangle with line tickness of 1px wide

    g.setColour(Colours::white);
    g.strokePath(responseCurve, PathStrokeType(2.f));// draws rounded rectangle with line tickness of 2px wide
}
